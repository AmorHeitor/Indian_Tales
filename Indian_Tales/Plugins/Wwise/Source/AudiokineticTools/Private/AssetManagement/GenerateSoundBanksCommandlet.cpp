...
int32 UGenerateSoundBanksCommandlet::Main(const FString& Params)
{
	...
	if (PlatformValue)
	{
		...
		{
			if (wwiseProjectInfo.SupportedPlatforms().Contains(name))
			{
	...
	else
	{
		...
		{
			InitParameters.Platforms.Add(platform);
		}
	}
	...
	const FString* BanksValue = ParamVals.Find(PlatformsSwitch);
	if (BanksValue)
	{
