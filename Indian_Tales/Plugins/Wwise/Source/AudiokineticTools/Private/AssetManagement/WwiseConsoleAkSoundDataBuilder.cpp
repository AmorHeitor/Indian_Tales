...
void WwiseConsoleAkSoundDataBuilder::runWwiseConsole()
{
	...
	FString BankNamesToGenerate = fillAudioBankInfoMap(audioBankInfoMap, FillAudioBankInfoKind::AssetName);
	if (!BankNamesToGenerate.IsEmpty())
	{
		wwiseConsoleArguments += TEXT(" --bank ") + BankNamesToGenerate;
	}
...

	{
		...
		if (isUsingNewAssetManagament)
		{
			...
			{
				auto Value = eventMapEntry.Value;
				if (!Value->RequiredBank)
				{
					auto bankName = guidToBankName(eventMapEntry.Key);
					auto line = bankName + TEXT("\t\"") + Value->GetName() + TEXT("\"\tEvent\tStructure") + LINE_TERMINATOR;
					fileWriter->Write(reinterpret_cast<const uint8*>(TCHAR_TO_UTF8(*line)), line.Len());

					if (Value->NeedsRebuild())
					{
		...
		if (isUsingNewAssetManagament)
		{
			...
			{
				auto Value = auxBusMapEntry.Value;
				if (!Value->RequiredBank)
				{
					auto bankName = guidToBankName(auxBusMapEntry.Key);
					auto line = bankName + TEXT("\t-AuxBus\t\"") + Value->GetName() + TEXT("\"\tStructure") + LINE_TERMINATOR;
					fileWriter->Write(reinterpret_cast<const uint8*>(TCHAR_TO_UTF8(*line)), line.Len());
...
bool WwiseConsoleAkSoundDataBuilder::readBankData(FByteBulkData& bulkBankData, const FString& bankFile, IPlatformFile& fileManager, FCriticalSection* DataLock)
{
	...
	newBankData.Reserve(fileSize);
	bankFileReader->Read(newBankData.GetData(), fileSize);
	bool changed = false;
	{
		FScopeLock autoLock(DataLock);
		int64 currentSize = bulkBankData.GetBulkDataSize();
		void* currentBankData = bulkBankData.Lock(EBulkDataLockFlags::LOCK_READ_WRITE);
		if (currentSize != fileSize || FMemory::Memcmp(newBankData.GetData(), currentBankData, FMath::Min(fileSize, bulkBankData.GetBulkDataSize()) != 0))
		{
			void* rawBankData = bulkBankData.Realloc(fileSize);
			FMemory::Memcpy(rawBankData, newBankData.GetData(), fileSize);
			changed = true;
		}
...
void WwiseConsoleAkSoundDataBuilder::processFile(const FString& FullPath, const FString& GeneratedSoundBanksFolder)
{
	...
	UAkAudioEvent* const* akAudioEventIt = nullptr;
	UAkAuxBus* const* akAuxBusIt = nullptr;
	UAkAudioBank* const* akAudioBankIt = nullptr;

	UAkAudioBank* foundakAudioBank = nullptr;

	if (isUsingNewAssetManagement)
	{
		auto bankGuid = bankNameToGuid(fileName);
			...
			auto readBankDataTask = FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, FullPath, platform, requiredData]
				{
					...
					{
						if (sharedThis->readBankData(requiredData->eventPlatformData->Data, FullPath, *sharedThis->platformFile, &(requiredData->akAudioEvent->BulkDataWriteLock)))
						{
							sharedThis->markAssetDirty(requiredData->akAudioEvent);
							sharedThis->markAssetDirty(requiredData->eventPlatformData);
						}
					}
				}, GET_STATID(STAT_ReadBankData), fetchPlatformDataTask);
			...
			if (auxBusPlatformData)
			{
				if (isBank)
				{
					auto readBankTask = FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, akAuxBus, auxBusPlatformData, FullPath] {
						if (sharedThis->readBankData(auxBusPlatformData->Data, FullPath, *sharedThis->platformFile, &(akAuxBus->BulkDataWriteLock)))
						{
							sharedThis->markAssetDirty(akAuxBus);
						}
					}, GET_STATID(STAT_ReadBankData));
			...
			auto readBankDataTask = FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, FullPath, platform, requiredData]
				{
					...
					{
						if (sharedThis->readBankData(requiredData->audioBankPlatformData->Data, FullPath, *sharedThis->platformFile, &(requiredData->akAudioBank->BulkDataWriteLock)))
						{
	...
	else if (fileName == AkInitBankName || fileName == AkPluginInfoFileName)
	{
		...
		if (isBank)
		{
			auto readBankTask = FFunctionGraphTask::CreateAndDispatchWhenReady([sharedThis, initBank, initBankPlatformData, FullPath] {
				if (sharedThis->readBankData(initBankPlatformData->Data, FullPath, *sharedThis->platformFile, &(initBank->BulkDataWriteLock)))
				{
