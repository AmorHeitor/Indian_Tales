...
void SLegacyGenerateSoundBanks::PopulateList()
{
	...
	for (const auto& Platform : AvailablePlatformNames)
	{
		...
		if (projectInfo.SupportedPlatforms().Contains(PlatformInfo->WwisePlatform) && !PlatformNames.ContainsByPredicate([PlatformInfo](TSharedPtr<FString> Platform) { return PlatformInfo->WwisePlatform == *Platform; }))
		{
