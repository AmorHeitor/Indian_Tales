...
void SGenerateSoundBanks::PopulateList(void)
{
	...
	for (const auto& Platform : AvailablePlatformNames)
	{
		...
		if (!PlatformNames.ContainsByPredicate([PlatformInfo](TSharedPtr<FString> Platform) { return PlatformInfo->WwisePlatform == *Platform; }) && wwiseProjectInfo.SupportedPlatforms().Contains(PlatformInfo->WwisePlatform))
		{
