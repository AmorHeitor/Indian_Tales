...
)
{
	...
	AKRESULT eResult = UnloadBank(in_Bank->GetName());
	if (eResult == AK_Success && AkBankManager)
	{
...
)
{
	if (m_bSoundEngineInitialized && in_Bank)
	{
		if (AkBankManager)
		{
			...
			if (cbInfo)
			{
				return AK::SoundEngine::UnloadBank(TCHAR_TO_AK(*(in_Bank->GetName())), nullptr, FAkBankManager::BankUnloadCallback, cbInfo);
			}
		}
		else
		{
			return AK::SoundEngine::UnloadBank(TCHAR_TO_AK(*(in_Bank->GetName())), nullptr, in_pfnBankCallback, in_pCookie);
		}
	}
...
{
	AsyncTask(ENamedThreads::GameThread, [this, EventID, PlayingID]() {
